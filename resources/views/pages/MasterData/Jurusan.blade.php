...
@section('content')
    ...
    <x-modal-form id="AddEditData" title="labelAddEdit">
        <div class="modal-body">
            <div class="row">
                <x-form-group class="col-sm-12 col-md-12" label="Jurusan" name="nama_jurusan" required />
            </div>
        </div>
...
@push('scripts')
    <script type="text/javascript">
        ...
        Refresh = function() {
            if (!$.fn.DataTable.isDataTable(id_tbl)) {
                ...
                table = PDataTables(dtu, [{
                    ...
                    render: function(data, type, row, meta) {
                        let html = "";
                        html += btnDataTable("Edit Jurusan", "btn-outline-primary edit",
                            "fa fa-edit btn-outline-primary", true);
                        html += btnDataTable("Delete Jurusan", "btn-outline-danger delete",
                ...
                table.on('click', '.delete', function() {
                    ...
                    $("#FDelData p").html("Are you sure to delete data jurusan <b>" + data.nama_jurusan +
                        "</b> ?");
                    ShowModal("MDelData");
                });
            } else {
        ...
        ShowData = function(act = "Add", data = "") {
            ...
            $("h4[labelAddEdit]").text(act + " Data Jurusan");
            processData = {
                action: act,
                kd_jurusan: (act == "Add" ? "" : data.kd_jurusan),
                nama_jurusan: (act == "Add" ? "" : data.nama_jurusan)
            };
            $(form_id + " [name='nama_jurusan']").val(processData.nama_jurusan).change();
        ...
        function Save() {
            ...
            if ($(form_id).parsley().validate()) {
                Loader("show");
                processData.nama_jurusan = $(form_id + " [name='nama_jurusan']").val();
                let data = {
                    url: $apiUrl + "MasterData/Jurusan/Save",
                    param: processData
                };
        ...
        Delete = function() {
            Loader("show");
            let data = {
                url: $apiUrl + "MasterData/Jurusan/Delete",
                param: processData
            };
            SendAjax(data, function(result) {
                MessageNotif(result.message, "success");
